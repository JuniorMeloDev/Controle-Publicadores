...
const getCurrentServiceYear = (monthIndex = new Date().getMonth(), year = new Date().getFullYear()) => {
  // Se for Setembro (8) a Dezembro (11), o ano de serviço é o ano final do ciclo
  return monthIndex >= 8 ? year + 1 : year;
};
...
const getPreviousMonthAndYear = () => {
  ...
  const anoCalendario = data.getFullYear(); // Pega o ano correto (ex: Jan 26 -> Dez 25)
  ...
  // O ano de serviço deve ser calculado com base no ANO CALENDÁRIO do mês anterior
  const ano = getCurrentServiceYear(mesIndex, anoCalendario);

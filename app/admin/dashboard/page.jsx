...
import { Users, Calendar, ArrowRight, TrendingUp, FileText, CheckCircle, Clock, Loader2, X } from 'lucide-react';
...
export default function Dashboard() {
  ...
  // ESTADO DE FILTRO POR GRUPO
  const [selectedGroup, setSelectedGroup] = useState('');
  
  // NOVO ESTADO DE FILTRO POR STATUS
  ...
  useEffect(() => {
    ...
    // Sempre reseta os filtros de Group e Status ao mudar o mês/ano
    setSelectedGroup(''); 
    setSelectedStatus('');
  ...
  // Filtra os relatórios exibidos com base no grupo e no status selecionados
  const filteredRelatorios = useMemo(() => {
    ...
    return list;
  }, [statusRelatorios, selectedGroup, selectedStatus]);
  ...
  const availableYears = useMemo(() => {
    ...
    // O ano de término do ciclo de serviço ATUAL (e.g., Nov 2025 -> 2026)
    ...
    // REGRA DO USUÁRIO: Incluir o próximo ano de serviço SÓ SE for Setembro (índice 8) ou depois.
    if (monthIndex >= 8) {
        // Se estamos em Setembro de 2026, currentEndYear é 2027.
        // Adiciona 2028 (fim do próximo ciclo).
        yearsList.add(currentEndYear + 1); 
    }
  ...
  return (
    <DashboardLayout>
      <div className="space-y-8">
        ...
        {/* --- Grid de Acesso Rápido (Omitido por brevidade) --- */}
        <div className="mt-8">
      ...
      <Sheet open={isSheetOpen} onOpenChange={(open) => {
          setIsSheetOpen(open);
          // Reseta os filtros de Group e Status ao fechar, mantendo o mês/ano
          if (!open) {
             setSelectedGroup(''); 
             setSelectedStatus('');
          }
        ...
        <SheetContent side="right" className="w-full sm:max-w-lg md:max-w-xl lg:max-w-2xl p-0 focus:outline-none flex flex-col bg-white">
          <SheetHeader className="p-6 border-b border-gray-200 bg-gray-50/50">
          ...
          {/* Controles de Filtro */}
          <div className="p-6 pt-4 border-b border-gray-100 flex flex-col gap-4 items-center shrink-0">
             ...
             <div className="flex gap-4 w-full">
                <div className="flex-1">
                    ...
                    >
                        <option value="">Todos os Grupos ({statusRelatorios.length})</option>
                        {gruposList.map(g => (
                            <option key={g} value={g}>{g}</option>
                        ))}
                        {/* Adicionar a opção 'Sem Grupo' se houver publicadores sem grupo, o que é detectado na API */}
             ...
             {loadingStatus ? (
                ...
                <div className="space-y-4">
                    <p className="text-sm font-semibold text-gray-700">
                        {enviados} Enviados / {pendentes} Pendentes (Exibindo: {filteredRelatorios.length})
                    </p>
                    
                    {/* Lista Agrupada por Status (Pendentes primeiro) E ORDENADA POR NOME */}
                    {[...filteredRelatorios]
                        .sort((a, b) => {
                            // 1. Prioriza Pendentes (Z-A)
                            if (a.status !== b.status) {
                                // 'Pendente' vem antes de 'Enviado' (P > E alfabeticamente reverso)
                                return b.status.localeCompare(a.status);
                            }
                            
                            // 2. Dentro do mesmo status, ordena por Nome Completo alfabeticamente (A-Z)
                            return a.nome_completo.localeCompare(b.nome_completo);
                        }) 
                        .map(pub => (
                        <div key={pub.id} className="flex items-center justify-between p-3 border-b border-gray-100 hover:bg-gray-50 transition-colors">
